---
name: no-use-effect
description: Enforce the no-useEffect rule when writing or reviewing React code. ACTIVATE when writing React components, refactoring existing useEffect calls, reviewing PRs with useEffect, or when an agent adds useEffect "just in case." Provides five replacement patterns and the useMountEffect escape hatch.
---
...
- Origin: [Alvin Sng](https://x.com/alvinsng/status/2033969062834045089)
...
1. Determine what the effect is doing — deriving state, fetching data, responding to an event, syncing with an external system, or resetting state.
2. Use the five rules below to pick the right replacement.
...
## Rule 1: Derive state inline — don't sync it
...
## Rule 2: Use a query library for data fetching
...
## Rule 4: useMountEffect for external system sync
...
## Rule 5: Use key prop for resetting state
